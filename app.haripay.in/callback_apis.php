...
include 'include.php';
$RequestStatus = $_METHOD['SSS'];
$ClientId = $_METHOD['RRR'];
$OperatorRef = $_METHOD['OOO'];
...
if ($ClientId != "") {
    ...
    if ($row == 1) {
        ...
        if ($RequestStatus == 'SUCCC') {
            $status = 'Success';
        } else if ($RequestStatus == 'FADDD') {
            $status = 'Failed';
        }
        $o = $factory->get_object($res[0]['user_id'], "users", "user_id");
         $o1->updated_at = todaysDate();
        
        $o1->response_url = serialize($_METHOD);
        if ($status == "Success") {
            $o1->status = "Success";
            $o1->opid = $OperatorRef;
