...
if ($_POST['ipay_send_updte'] == '1') {
    ...
    if ($_SESSION['ipay_customer_id'] > 0) {
        $o4 = $factory->get_object($_SESSION['ipay_customer_id'], "ipay_user", "ipay_user_id");
    }
    ...
    if ($amount_slab > 1) {
        $sql_comm = "Select * from dmr_commission where  plan_id = '".$o->plan_id."' and (start_amount <= '5000' and end_amount >='5000' ) and dmr_type = 'Instant Pay'  and is_active = 1";
        $res_comm = getXbyY($sql_comm);
        $tt_amount = 0;
        for ($i = 0; $i < $amount_slab; $i++) {
            if ($i == ($amount_slab - 1)) {
                $pending_amount = $dezire_amount - (5000 * $i);
                $sql_comm = "Select * from dmr_commission where  plan_id = '".$o->plan_id."' and (start_amount <= '" . $pending_amount . "' and end_amount >='" . $pending_amount . "' ) and dmr_type = 'Instant Pay'  and is_active = 1";
                $res_comm = getXbyY($sql_comm);
        ...
        $sql_comm = "Select * from dmr_commission where   plan_id = '".$o->plan_id."' and (start_amount <= '" . $dezire_amount . "' and end_amount >='" . $dezire_amount . "' ) and dmr_type = 'Instant Pay'  and is_active = 1";
        $res_comm = getXbyY($sql_comm);
