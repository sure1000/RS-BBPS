...
$trigger_transaction_type = "(transaction_type = 'Recharge' or transaction_type = 'R Offer Check' or transaction_type = 'User Info Check')";
$trigger_status = "1=1";
...
$sql_total_amount = "Select sum(amount) as total_amount, status from wallet where $triggers group by status";
$res_total_amount = getXbyY($sql_total_amount);
$row_total_amount = count($res_total_amount);

$total_amount = 0;
for ($i = 0; $i < $row_total_amount; $i++) {
	if ($res_total_amount[$i]['status'] != "Failed") {
		$total_amount = $total_amount + $res_total_amount[$i]['total_amount'];
	}
}
...
if ($row_transactions > 0) {
	...
	for ($i = 0; $i < $row_transactions; $i++) {
		...
		if ($res_transactions[$i]['status'] == "Failed") {
			$status = "<span class='red'>Failed</span>";
			$res_transactions[$i]['user_new_balance'] = $res_transactions[$i]['user_old_balance'];
	...
	$data[$i]['transaction_type'] = "Total";
	$data[$i]['api'] = "";
	$data[$i]['status'] = "Success/Pending";
	$data[$i]['user_old_balance'] = "";
	$data[$i]['amount'] = round($total_amount, 2);
	$data[$i]['user_new_balance'] = "";
} else {
	$data = array();
}
