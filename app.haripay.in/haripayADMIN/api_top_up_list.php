...
$triggers = $trigger_from_date . " and " . $trigger_to_date . " and " . $trigger_search_val . " and " . $trigger_api_id;
...
$sql_total = "Select count(wallet_id) as total_transactions from wallet where $triggers and transaction_type = 'API Top up' order by wallet_id";
$res_total = getXbyY($sql_total);

$sql_transactions = "Select * from wallet where $triggers and transaction_type = 'API Top up' order by wallet_id limit $row ,$length";
...
if ($row == 0) {
	...
	$sql_turnover = "Select sum(api_amount) as total_amount from wallet where $triggers and transaction_type = 'API Top up' and wallet_id < " . $res_transactions[0]['wallet_id'] . " group by status";
...
if ($row_transactions > 0) {
	...
	for ($i = 0; $i < $row_transactions; $i++) {
		...
		if ($res_transactions[$i]['status'] == "Failed") {
			...
			$class = "green";
			$new_balance = round($old_balance + $res_transactions[$i]['api_amount'], 2);
		}
		...
		$data[] = array(
			...
			"amount" => "<span class=" . $class . ">" . $sign . "<i class='fa fa-rupee-sign'></i> " . $res_transactions[$i]['api_amount'] . "</span>",
