...
if ($row == 0) {
	...
	*/
	...
	$sql_turnover = "Select sum(api_amount) as total_amount, status from wallet where $triggers and wallet_id < " . $res_transactions[0]['wallet_id'] . " group by status";
	$res_turnover = getXbyY($sql_turnover);
...
if ($row_transactions > 0) {
	...
	for ($i = 0; $i < $row_transactions; $i++) {
		...
		if ($res_transactions[$i]['status'] == "Failed") {
			...
			$class = "green";
			$new_balance = $old_balance + $res_transactions[$i]['api_amount'];
		}
