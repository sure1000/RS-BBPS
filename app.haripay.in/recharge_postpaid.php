...
if ($_POST['postpaid_updte'] == '1') {
	$o1 = new wallet;
	$o1->amount = $_POST['postpaid_amount'];
	$o1->total_amount =$_POST['postpaid_amount'];
	$o1->mobile_number = $_POST['postpaid_number'];
	$o1->provider_id = $_POST['postpaid_provider'];
	$o1->circle_name = $_POST['postpaid_circle'];
	$o1->circle_id = "0";
	$service_id = "2";
	$userservice_check = check_userServices('Postpaid',$o);
 ...
 if($o->plan_id > 0){
	...
	if ($o1->amount <= $o->amount_balance) {
       ...
       $sql_circle = "Select circle_code from api_circle_code where api_id = '" . $api_id . "' and circle_name = '" . $o1->circle_name . "' and is_active = '1'";
	    ...
	    $result['error'] = 1;
        $result['error_msg'] = "You are not authorized for postpaid service";
 }
} else {
	$result['error'] = 1;
