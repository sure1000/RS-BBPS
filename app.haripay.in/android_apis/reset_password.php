...
if ($updte == 1) {
	...
	$o->new_password = $_POST['verification_code'];
	$o->password = cpassword($_POST['new_password']);

	$sql_check = "Select user_id from users where (email = '" . $o->email . "' or mobile = '" . $o->email . "') and otp = '" . $o->new_password . "' and is_active > 0";
	...
	if ($row_check == 1) {
		...
		$result['error'] = "0";
		$result['error_msg'] = $res_site[0]['site_url'] . " Password Changed Successfully";

		$email_from = $res_site[0]['email'];
		$email_to = $o->email;
		$email_subject = "Password Reset for " . $res_site[0]['site_name'];
        $email_message="Your One Time OTP is ".$o->otp;
