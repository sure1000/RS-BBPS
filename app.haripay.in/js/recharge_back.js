...
function recharge_now(form_name) {
    $('#recharge_data').modal('hide');
    if (form_name == "prepaid") {
        $('#prepaid').hide();
        $('#mobile_processing').show();
        recharge_prepaid();
    } else if (form_name == "postpaid") {
        $('#postpaid').hide();
        $('#mobile_processing').show();
        recharge_postpaid();
    } else if (form_name == "dth") {
        $('#dth').hide();
        $('#dth_processing').show();
        recharge_dth();
    } else if (form_name == "landline") {
        $('#landline').hide();
        $('#landline_processing').show();
        recharge_landline();
    } else if (form_name == "dth_activation") {
        $('#dth_activation').hide();
        $('#dth_processing').show();
        recharge_dth_activation();
    } else if (form_name == "water") {
        $('#water').hide();
        $('#utility_processing').show();
        recharge_water();
    } else if (form_name == "gas") {
        $('#gas').hide();
        $('#utility_processing').show();
        recharge_gas();
    } else if (form_name == "school_fee") {
        $('#school_fee').hide();
        $('#school_processing').show();
        recharge_school_fee();
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (result['error'] == 0) {
                ...
                $('#electricity_submit').val('Fetch Bill');
                swal({title: "Error", text: result['error_msg'], icon: "error", buttons: false, timer: 2000});
            } else {
                ...
                var dateTod = dd + '-' + mm + '-' + yyyy;

                var dueDte = result['date'];
                ...
                $('#customer_name').html(result['data']['CustomerName']);
                $('#CustomerName').val(result['data']['CustomerName']);
                $('#bill_number').html(result['data']['BillNumber']);
                $('#bill_number_elec').val(result['data']['BillNumber']);
                $('#elec_amount').val(result['data']['Amount']);
                $('#amount_elec').html(result['data']['Amount']);
                $('#DueDate_elc').html(result['date']);
                $('#requestId').val(result['ResultID']);
                $('#due_date').val(result['date']);
                $('#state').html(result['state']);
                $('#provider_elec').html(result['circle']);
                $('#circle_logo').html($('#Electricity_provider_logo').html());
                $('#commission_data_elc').html(result['commission_amount']);
                $('#tds_data_elec').html(result['tds_amt']);
                $('#gst_data_elec').html(result['gst_amt']);
                $('#net_profit').html(result['net_profit']);
                $('#total_payable_elec').html(result['pay_amount']);
...
//    $('#recharge_data').modal('show');
//    $('#recharge_logo').html($('#postpaid_provider_logo').html());
//    $('#number_data').html($('#postpaid_number').val());
//    $('#amount_data').html("<b><i class='fa fa-rupee-sign'></i> " + $('#postpaid_amount').val() + "</b>");
//    $('#recharge_type').val('postpaid');
//    commission_tax($("#postpaid_amount").val());
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (result['error'] == 1) {
                ...
                $('#' + service + '_provider_logo').html(result['logo']);
                $('#commission_amount').val(result['commission_amount']);
                $('#commission_percentage').val(result['commission_percentage']);
                if (service == "prepaid") {
                    get_prepaid_plans("combo", service);
                } else if (service == "dth") {
                    get_prepaid_plans("Plan", service);
                }
                $('#' + service + '_plans_parent').find('li').removeClass('active');
                $('#' + service + '_plans_parent').find('a').removeClass('active');
                $('#' + service + '_plans_primary').addClass('active');
                $('#' + service + '_plans_primary a').addClass('active');
            }
...
//    if (parseInt(mobile) < 6000000000 || parseInt(mobile) > 9999999999) {
//        swal({title: "Error", text: "Incorrect Mobile Number", icon: "error", buttons: false, timer: 2000});
    // $('#' + service + '_number').val('');
...
function commission_tax(amount) {
    var comm_type = $('#commission_amount').val();
    var comm_percentage = $('#commission_percentage').val();
    var total_commission_amount = 0;
    var total_payable = 0;
    if (comm_type == "Commission Percentage") {
        total_commission_amount = roundToTwo((parseFloat(comm_percentage) / 100) * parseFloat(amount));
        total_payable = roundToTwo(parseFloat(amount) - parseFloat(total_commission_amount));
    } else if (comm_type == "Commission Flat") {
        total_commission_amount = comm_percentage;
        total_payable = roundToTwo(parseFloat(amount) - parseFloat(total_commission_amount));
    } else if (comm_type == "Surcharge Percentage") {
        total_commission_amount = roundToTwo((parseFloat(comm_percentage) / 100) * parseFloat(amount));
        total_payable = roundToTwo(parseFloat(amount) + parseFloat(total_commission_amount));
    } else if (comm_type == "Surcharge Flat") {
        total_commission_amount = comm_percentage;
        total_payable = roundToTwo(parseFloat(amount) + parseFloat(total_commission_amount));
    }
    var net_profit = 0;
    net_profit = roundToTwo(parseFloat(total_commission_amount));
    $('#commission_data').html("<b><i class='fa fa-rupee-sign'></i> " + total_commission_amount + "</b>");
    $('#total_payable').html("<b><i class='fa fa-rupee-sign'></i> " + total_payable + "</b>");
    $('#net_profit').html("<b><i class='fa fa-rupee-sign'></i> " + net_profit + "</b>");
    $('#recharge_now_amount').html(total_payable);
}
