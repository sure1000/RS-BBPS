...
$sql_del = "Delete from provider_plans where service_type = 'Dth'";
$set_del = setXbyY($sql_del);

$sql = "Select * from providers where is_active = 1 and service = 'DTH'";
$res = getXbyY($sql);
$rows = count($res);
...
for ($i = 0; $i < $rows; $i++) {
	...
	$provider_code = get_api_provider_code($res[$i]['provider_id'], "mplan");

	$result = mplan_plans_dth($provider_code);
	...
	foreach ($objects as $key => $value) {
		...
		for ($l = 0; $l < $total_ptype; $l++) {
			$pr = (get_object_vars($objects[$ptype][$l]->rs));

			foreach ($pr as $key1 => $value) {
				$o1->validity = $key1;
				$o1->price = $pr[$key1];
			}
			$o1->details = "<b>" . $objects[$ptype][$l]->plan_name . "</b><br />" . addslashes($objects[$ptype][$l]->desc);
