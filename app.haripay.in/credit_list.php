...
$trigger_transaction_type = "(transaction_type = 'Recieve Money' or transaction_type = 'Reverse')";
$trigger_status = "1=1";
...
$triggers = $trigger_transaction_type . " and " . $trigger_status . " and " . $trigger_from_date . " and " . $trigger_to_date . " and " . $trigger_search_val;
...
$sql_total = "Select count(wallet_id) as total_transactions from wallet where user_id = " . $o->user_id . " and comment = 'Credit Update' and $triggers";
$res_total = getXbyY($sql_total);

$sql_transactions = "Select * from wallet where user_id = " . $o->user_id . " and comment = 'Credit Update' and $triggers order by wallet_id DESC limit $row ,$length";
...
if ($row == 0) {
	//$old_balance = 0;
	$new_balance = $o->credit_amount;
} else {
	$sql_turnover = "Select transaction_type, sum(amount) as commission from wallet where user_id = '" . $o->user_id . "' and comment = 'Credit Update' and $triggers and wallet_id <= " . $res_transactions[0]['wallet_id'];
	$res_turnover = getXbyY($sql_turnover);
	$row_turnover = count($res_turnover);

	//$old_balance = round($res_turnover[0]['commission'], 2);
	$new_balance = round($res_turnover[0]['commission'], 2);
...
if ($row_transactions > 0) {
	...
	for ($i = 0; $i < $row_transactions; $i++) {
		...
		if ($class == "red") {
			...
			$old_balance = $new_balance + $res_transactions[$i]['amount'];
		} else {
			$sign = "+";
			//$new_balance = $old_balance + $res_transactions[$i]['amount'];
			$old_balance = $new_balance - $res_transactions[$i]['amount'];
		...
		$ttype = "<b>" . $res_transactions[$i]['transaction_type'] . "</b> -"
			. "<b>" . $res_transactions[$i]['mobile_number'] . " (" . $res_transactions[$i]['cash_credit'] . ")</b><br />"
